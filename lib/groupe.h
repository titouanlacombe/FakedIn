...
// Retourne la worker qui porte l'index donné.
worker* g_index(groupe* g, int const index);

// Détermine si deux personnes sont amies.
// Retourne vrai si et seulement si la worker index_a compte la worker index_b parmi ses ami(e)s et que la worker index_b compte la worker index_a parmis ses ami(e)s.
...
// Détermine le(la) meilleur(e) ami(e) d'une worker.
// Une worker a un(e) meilleur(e) ami(e) si le(le) premier(ère) ami(e) dans sa liste d'amis a cette même worker comme premier(ère) ami(e).
...
// Une worker a une amitié unidirectionnelle si elle compte dans sa liste d'amis une worker qui ne l'a pas dans sa liste d'amis.
// Retourne vrai si et seulement si la worker index_a compte la worker index_b parmi ses ami(e)s et que la worker index_b ne compte pas la worker index_a parmis ses ami(e)s.
// g = {[1,Alice,3;2] [2,Bob,3;1] [3,Charlie,2]}
...
// Détermine s'il existe une chaîne d'amis entre la worker index_a et la worker index_b.
...
// Enlève une worker du groupe.
// Tous les liens d'amitié vers cette worker supprimée doivent eux aussi être supprimés.
